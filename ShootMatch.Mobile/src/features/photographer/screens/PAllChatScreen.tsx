        ...
        renderItem={({ item: c, index }) => (
          <Animated.View entering={FadeInDown.duration(400).delay(index * 60)}>
            <Pressable
              ...
              onPress={() => navigation.navigate('Chat', { conversationId: c.id, name: `Khách #${(c.customerId ?? '').slice(0, 6)}` })}
            >
              <Image source={{ uri: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=400&q=80' }} style={styles.avatar} />
              <View style={styles.rowContent}>
                <View style={styles.rowTop}>
                  <Text style={styles.rowName} numberOfLines={1}>Khách hàng #{(c.customerId ?? '').slice(0, 6)}</Text>
