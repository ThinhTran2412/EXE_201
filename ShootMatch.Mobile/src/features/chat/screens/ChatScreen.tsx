...
import { getCustomerProfile } from '../../customer/api';
...
import {
  ...
  isPeerOnline,
  ONLINE_THRESHOLD_MS,
} from '../utils/messageClusters';
import ChatMessageClusterView from '../components/ChatMessageClusterView';
...
export default function ChatScreen() {
  ...
  const peerOnline = isPeerOnline(undefined, peerLiveUntil);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      cleanup = ChatHub.onReceiveMessage((incoming) => {
        if (incoming.senderRole === 'photographer') {
  ...
  function handleCallPress() {
    Alert.alert('Bắt đầu cuộc gọi', 'Chọn loại cuộc gọi:', [
            ...
            renderItem={({ item }) => {
              ...
              return (
                <ChatMessageClusterView
                  ...
                  onPress={() => setExpandedClusterId((prev) => (prev === item.cluster.id ? null : item.cluster.id))}
                  myAvatar={myAvatar}
