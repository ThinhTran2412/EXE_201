...
public sealed class EfNotificationRepository(ShootMatchDbContext db) : INotificationRepository
{
    ...
    {
        await db.Set<AppNotificationRecord>().AddAsync(new AppNotificationRecord
        {
            Id = notification.Id,
    ...
    public async Task<IReadOnlyList<AppNotification>> GetForRecipientAsync(
        ...
        => (await db.Set<AppNotificationRecord>().AsNoTracking()
    ...
    public Task<int> GetUnreadCountAsync(Guid recipientId, string recipientRole, CancellationToken cancellationToken = default)
        => db.Set<AppNotificationRecord>().CountAsync(
    ...
    {
        var row = await db.Set<AppNotificationRecord>().FirstOrDefaultAsync(
    ...
    {
        var rows = await db.Set<AppNotificationRecord>()
